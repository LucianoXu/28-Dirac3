...
def test_BRA_ADD_1():
    with wolfram_backend.wolfram_session():
        a = parse(r''' ZEROB ADDB B ADDB ZEROB ''')
        b = parse(r''' B ''')
        assert trs.normalize(a) == trs.normalize(b)
...
def test_BRA_MUL_1():
    with wolfram_backend.wolfram_session():
        a = parse(r''' B MLTB ZEROO ''')
        b = parse(r''' ZEROB ''')
        assert trs.normalize(a) == trs.normalize(b)

def test_BRA_MUL_2():
    with wolfram_backend.wolfram_session():
        a = parse(r''' ZEROB MLTB O ''')
        b = parse(r''' ZEROB ''')
        assert trs.normalize(a) == trs.normalize(b)

def test_BRA_MUL_3():
    with wolfram_backend.wolfram_session():
        a = parse(r''' B0 MLTB ONEO ''')
        b = parse(r''' B0 ''')
        assert trs.normalize(a) == trs.normalize(b)
...
def test_BRA_TSR_1():
    with wolfram_backend.wolfram_session():
        a = parse(r''' ZEROB TSRB B ''')
        b = parse(r''' ZEROB ''')
        assert trs.normalize(a) == trs.normalize(b)

def test_BRA_TSR_2():
    with wolfram_backend.wolfram_session():
        a = parse(r''' B TSRB ZEROB ''')
        b = parse(r''' ZEROB ''')
        assert trs.normalize(a) == trs.normalize(b)
...
def test_OPT_OUTER_1():
    with wolfram_backend.wolfram_session():
        a = parse(r''' ZEROK OUTER B0 ''')
        b = parse(r''' ZEROO ''')
        assert trs.normalize(a) == trs.normalize(b)

def test_OPT_OUTER_2():
    with wolfram_backend.wolfram_session():
        a = parse(r''' K0 OUTER ZEROB ''')
        b = parse(r''' ZEROO ''')
        assert trs.normalize(a) == trs.normalize(b)
...
def test_OPT_ADD_1():
    with wolfram_backend.wolfram_session():
        a = parse(r''' ZEROO ADDO O ADDO ZEROO ''')
        b = parse(r''' O ''')
        assert trs.normalize(a) == trs.normalize(b)
...
def test_OPT_MUL_1():
    with wolfram_backend.wolfram_session():
        a = parse(r''' ZEROO MLTO O0 ''')
        b = parse(r''' ZEROO ''')
        assert trs.normalize(a) == trs.normalize(b)

def test_OPT_MUL_2():
    with wolfram_backend.wolfram_session():
        a = parse(r''' O0 MLTO ZEROO ''')
        b = parse(r''' ZEROO ''')
        assert trs.normalize(a) == trs.normalize(b)

def test_OPT_MUL_3():
    with wolfram_backend.wolfram_session():
        a = parse(r''' ONEO MLTO O0 ''')
        b = parse(r''' O0 ''')
        assert trs.normalize(a) == trs.normalize(b)

def test_OPT_MUL_4():
    with wolfram_backend.wolfram_session():
        a = parse(r''' O0 MLTO ONEO ''')
        b = parse(r''' O0 ''')
        assert trs.normalize(a) == trs.normalize(b)
...
def test_OPT_TSR_1():
    with wolfram_backend.wolfram_session():
        a = parse(r''' ZEROO TSRO O ''')
        b = parse(r''' ZEROO ''')
        assert trs.normalize(a) == trs.normalize(b)

def test_OPT_TSR_2():
    with wolfram_backend.wolfram_session():
        a = parse(r''' O TSRO ZEROO ''')
        b = parse(r''' ZEROO ''')
        assert trs.normalize(a) == trs.normalize(b)
